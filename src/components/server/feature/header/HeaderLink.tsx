...
export async function HeaderLink({ number, text }: HeaderLinkProps) {
  return (
    <Link href={`#${text}`} className="group flex flex-col items-end">
      ...
      <span className="before:text-primary font-sfmono text-base font-semibold leading-none before:absolute before:-translate-x-5 before:transition-all before:content-['//'] group-hover:before:rotate-[-180deg]">
        {text}
      </span>
    </Link>
  )
